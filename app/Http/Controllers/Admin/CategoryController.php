...
class CategoryController extends Controller
{
    public function index(){
        $category = Category::orderBy('catetgory_id','desc') ->Paginate(5);
    ...
    public function edit($catetgory_id){
        $cat = Category::find($catetgory_id);
    ...
    $category = Category::find ($category_id);
    $category->delete();
    alert()->success('ลบข้อมูลสำเร็จแล้ว','ข้อมูลนี้ถูกลบแล้ว');
    return redirect()->route('c.index');
    }
}
